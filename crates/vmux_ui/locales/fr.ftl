...
tools-title = Outils
tools-search = Rechercher des paquets, agents, MCP, LSP et fichiers de configuration…
...
tools-provider-lsp-servers = Serveurs LSP
tools-provider-mcp-servers = Serveurs MCP
mcp-command-description = Gérer les serveurs MCP
mcp-linear-description = Tickets, projets et équipes
...
vault-recovery-key-create-failed = La clé de récupération n'a pas pu être créée.
vault-recovery-key-save = Enregistrez cette clé maintenant. Vmux ne peut pas l'afficher à nouveau.
vault-recovery-key-saved = Je l'ai enregistré
vault-recovery-key-placeholder = Coller la clé de récupération
vault-recovery-key-unlock = Déverrouiller
vault-recovery-key-invalid = Cette clé de récupération ne peut pas déverrouiller le coffre-fort.
vault-recovery-key-ready = Clé de récupération configurée
vault-recovery-key-upload-pending = La clé est enregistrée localement, mais le téléchargement a échoué. Gardez-le en sécurité et appuyez sur Sync pour réessayer.
vault-auto-sync = Sauvegarde automatique
vault-backup-failed = La sauvegarde automatique a échoué. Appuyez sur Sync pour réessayer.
vault-backup-failed-short = La sauvegarde a échoué
vault-recovery-key-copy-hint = Cliquez pour copier
vault-recovery-key-copied = Copié
vault-recovery-key-verify = Enregistrez-le dans votre gestionnaire de mots de passe, puis collez-le ci-dessous pour vérifier.
vault-recovery-key-verify-placeholder = Coller la clé de récupération enregistrée
vault-recovery-key-mismatch = La clé de récupération ne correspond pas.
...
vault-github = GitHub
vault-github-description = Créez un référentiel privé ou choisissez un coffre-fort existant.
vault-connect-github = Connectez-vous à GitHub
vault-connected-as = Connecté en tant que { $name }
vault-use-repository = Utiliser
vault-repository-name = Nom du référentiel
vault-cloud-folder = Dossier Nuage
vault-cloud-folder-description = Utilisez un dossier de Google Drive, iCloud Drive, Dropbox ou OneDrive.
vault-choose-folder = Choisir un dossier
vault-result-github-connected = GitHub · { common-done }
vault-result-folder-connected = Dossier · { common-done }
vault-private = Dépôt privé
vault-public-warning = L’historique du dépôt et les données chiffrées seront accessibles au public.
...
simulator-keyboard = Clavier
simulator-keyboard-placeholder = Saisissez ou collez dans l’iPhone
simulator-copy = Copier
simulator-paste = Coller
...
composer-branch = Branche
composer-attach-files = Joindre des fichiers (/upload)
composer-remove-attachment = Supprimer la pièce jointe
composer-choose-agent = Choisir un agent
composer-create-worktree = + Worktree
composer-create-worktree-title = Créer ou sélectionner un worktree pour ce projet
composer-status-running = En cours
composer-status-approval = Approbation
composer-status-starting = Démarrage
composer-status-error = Erreur
composer-status-subagents = Sous-agents actifs
composer-status-tasks-title = Tâches ouvertes
composer-status-tasks = { $count } tâches
composer-status-queued-title = Prompts en file
composer-status-queued = { $count } en file
composer-uncommitted-changes = Modifications non validées
composer-commits-ahead = Commits en avance sur l’amont
composer-no-git = Pas de Git
composer-git = Git
composer-git-repository = Dépôt Git
composer-branch-name = Branche { $branch }
...
command-partial-index = Seule une partie de ce projet est indexée
command-more-matches = {$shown} correspondances affichées sur {$total}
command-placeholder = Saisissez une URL, recherchez des onglets ou tapez > pour les commandes…
command-ex-placeholder = Saisissez une commande Ex…
palette-mode-ex = Ex
palette-mode-command = Commandes
palette-mode-slash = Commande
palette-mode-path = Fichiers
ex-write = Enregistrer le fichier
ex-write-quit = Enregistrer puis fermer
ex-quit = Fermer sans enregistrer
ex-quit-force = Fermer et abandonner les modifications
ex-nohighlight = Effacer la surbrillance de recherche
ex-delete = Supprimer la ligne
ex-yank = Copier la ligne
ex-substitute = Substituer dans la ligne
...
layout-project-forget = Oublier ce projet
layout-project-activate = Ouvrir les nouveaux onglets ici
...
common-collapse-all = Tout réduire
...
command-browser-open-goto-line = Navigateur > Barre > Atteindre la ligne
command-browser-open-indentation = Navigateur > Barre > Sélectionner l’indentation
command-browser-open-line-ending = Navigateur > Barre > Sélectionner la fin de ligne
command-browser-open-encoding = Navigateur > Barre > Sélectionner l’encodage
command-browser-open-reopen-with-encoding = Navigateur > Barre > Rouvrir avec l’encodage
command-browser-open-save-with-encoding = Navigateur > Barre > Enregistrer avec l’encodage
...
agent-change-model = Changer de modèle
...
agent-option-default = { $value } (par défaut)
agent-tool-calling = Appel d'un outil
...
editor-status-position = Lig { $line }, Col { $col }
editor-status-spaces = Espaces : { $width }
editor-status-tabs = Tabulation : { $width }
editor-status-goto-title = Atteindre la ligne
editor-status-goto-placeholder = Numéro de ligne
editor-status-pick-placeholder = Saisir pour filtrer
editor-status-indent-title = Sélectionner l'indentation
editor-status-eol-title = Sélectionner la séquence de fin de ligne
editor-status-encoding-title = Sélectionner l'encodage
editor-status-encoding-reopen = Rouvrir avec l'encodage
editor-status-encoding-save = Enregistrer avec l'encodage
editor-cannot-open = Ce fichier n’a pas pu être ouvert
editor-edit-failed = Le renommage a échoué
editor-go-to-declaration = Atteindre la déclaration
editor-go-to-type-definition = Atteindre la définition de type
editor-go-to-implementation = Atteindre les implémentations
editor-rename-symbol = Renommer le symbole
editor-change-all-occurrences = Modifier toutes les occurrences
editor-format-document = Formater le document
editor-format-selection = Formater la sélection
editor-code-action = Refactoriser…
editor-cut = Couper
editor-copy = Copier
editor-paste = Coller
editor-command-palette = Palette de commandes…
editor-unsaved = non enregistré
editor-find-placeholder = Rechercher
editor-find-no-results = Aucun résultat
editor-find-previous = Résultat précédent
editor-find-next = Résultat suivant
editor-find-close = Fermer la recherche
editor-find-regex = Utiliser une expression régulière
editor-find-case = Respecter la casse
editor-find-in-files-placeholder = Rechercher dans les fichiers
editor-find-whole-word = Mot entier
editor-search = Rechercher
editor-search-clear = Effacer les résultats
editor-search-idle = Appuyez sur Entrée pour rechercher
editor-search-summary = { $results } résultats dans { $files } fichiers
editor-search-summary-capped = Les { $results } premiers résultats dans { $files } fichiers
editor-show-explorer = Afficher l'explorateur
editor-show-search = Afficher la recherche
...
editor-explorer-empty = Aucun fichier
editor-open-editors = Éditeurs ouverts
editor-close-editor = Fermer l’éditeur
editor-outline = Plan
editor-close-directory = Fermer le répertoire
editor-breadcrumb = Fil d’Ariane
editor-breadcrumb-hidden = Afficher les dossiers masqués
editor-breadcrumb-empty = Rien ici
...
editor-rename-failed = Échec du renommage
...
git-stage-file = Indexer le fichier
git-unstage = Désindexer
git-confirm-discard = Confirmer l’abandon
git-discard-file = Abandonner les modifications
git-commit-message = message de commit
git-commit = Commit ({ $count })
git-push = ↑ Pousser
git-loading-diff = Chargement du diff…
git-no-changes = Aucune modification à afficher
git-stage-hunk = Indexer le bloc
git-revert-hunk = Rétablir le bloc
...
knowledge-search = Rechercher des connaissances
knowledge-no-match = Aucune note correspondante
knowledge-open-linked-note = Ouvrir la note liée
knowledge-create-linked-note = Créer une note liée
editor-properties = Propriétés
editor-no-properties = Aucune propriété
editor-add-tags = Ajouter des balises
editor-add-property = Ajouter une propriété
editor-change-property-type = Changer le type de propriété
editor-add-tag = Ajouter une balise
editor-add-item = Ajouter un article
editor-linked-note = Note liée
editor-property-value = Valeur
editor-delete-property = Supprimer la propriété
editor-property-kind-text = Texte
editor-property-kind-number = Nombre
editor-property-kind-checkbox = Case à cocher
...
agent-project-select = Sélectionner un projet
agent-project-choose = Choisir un projet
agent-project-choose-another = Choisir un autre dossier…
agent-project-none = Aucun projet
agent-project-loading-branches = Chargement des branches…
agent-project-no-branches = Aucune branche
agent-project-open-worktree = Ouvrir ce worktree
agent-project-branch-change = Modifications par rapport à la branche de base
agent-project-create-worktree = Créer un worktree sur cette branche
resume-column-agent = Agent
resume-column-project = Projet
resume-column-branch = Branche
resume-column-updated = Mis à jour
resume-age-now = maintenant
resume-age-minutes = { $count } min
resume-age-hours = { $count } h
resume-age-days = { $count } j
resume-age-weeks = { $count } sem
resume-age-months = { $count } mois
resume-age-years = { $count } a
