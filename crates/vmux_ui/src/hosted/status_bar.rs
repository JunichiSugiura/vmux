...
#[derive(Resource, Default)]
pub struct StatusUiChromeUnavailable(pub bool);
...
/// CEF warmup for the status Dioxus bundle when the loopback URL is ready (see [`StatusBarServerPlugin`]).
pub fn status_dioxus_warmup_should_spawn(world: &mut World) -> Option<String> {
    if std::env::var("VMUX_STATUS_DISABLE_CEF_WARMUP")
        .map(|s| s == "1" || s.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
    {
        return None;
    }
    let chrome = world.get_resource::<StatusUiChromeUnavailable>()?;
    if chrome.0 {
        return None;
    }
...
#[derive(Default)]
pub struct StatusBarServerPlugin;

impl ServePlugin for StatusBarServerPlugin {
    fn dioxus_warmup_descriptor() -> Option<DioxusWarmupDescriptor> {
        Some(DioxusWarmupDescriptor {
            surface: EmbeddedDioxusUiSurface::PaneChrome,
            name: "vmux_status_ui_warmup",
            should_spawn: status_dioxus_warmup_should_spawn,
        })
    }
}

impl Plugin for StatusBarServerPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_resource::<StatusUiBaseUrl>()
            ...
            .add_systems(
                Update,
                (
                    poll_status_url.in_set(DioxusUiWarmupSet::PollUrls),
...
impl VmuxHostedWebPlugin for StatusBarServerPlugin {
...
mod tests {
    ...
    fn status_bar_plugin_registers_in_app() {
        let mut app = App::new();
        app.add_plugins((ServerPlugin, StatusBarServerPlugin));
    }
}
