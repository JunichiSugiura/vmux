//! Native builds refresh **`dist/`** via **`dx build --platform web`** (Tailwind + wasm-bindgen bundled in dioxus-cli).
//! **Wasm** target builds are a no-op here; `dx` / `asset!` handle CSS when producing the bundle.
...
fn main() {
    ...
    if !needs_dist_rebuild(&manifest_dir, release) {
        return;
    }

    run_dx_web_bundle(&workspace_root, "vmux_status_bar", release, &[]);
    let public = dx_web_public_dir(&workspace_root, "vmux_status_bar", release);
...
fn needs_dist_rebuild(manifest_dir: &Path, release: bool) -> bool {
    let dist = manifest_dir.join("dist");
    ...
    let workspace_root = workspace_root_from_manifest_dir(manifest_dir);
    let dx_public = dx_web_public_dir(&workspace_root, "vmux_status_bar", release);
    let dx_wasm = dx_public.join("wasm").join("vmux_status_bar_bg.wasm");
    if dx_wasm.is_file() {
