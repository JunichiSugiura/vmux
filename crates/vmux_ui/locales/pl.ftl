...
mcp-command-description = Zarządzaj serwerami MCP
mcp-linear-description = Zgłoszenia, projekty i zespoły
...
simulator-keyboard = Klawiatura
simulator-keyboard-placeholder = Wpisz lub wklej na iPhonie
simulator-copy = Kopiuj
simulator-paste = Wklej
...
composer-branch = Gałąź
composer-attach-files = Dołącz pliki (/upload)
composer-remove-attachment = Usuń załącznik
composer-choose-agent = Wybierz agenta
composer-create-worktree = + Worktree
composer-create-worktree-title = Utwórz lub wybierz worktree dla tego projektu
composer-status-running = Działa
composer-status-approval = Zatwierdzenie
composer-status-starting = Uruchamianie
composer-status-error = Błąd
composer-status-subagents = Aktywne subagenty
composer-status-tasks-title = Otwarte zadania
composer-status-tasks = Zadania: { $count }
composer-status-queued-title = Prompty w kolejce
composer-status-queued = W kolejce: { $count }
composer-uncommitted-changes = Niezatwierdzone zmiany
composer-commits-ahead = Commity przed gałęzią nadrzędną
composer-no-git = Brak Git
composer-git = Git
composer-git-repository = Repozytorium Git
composer-branch-name = Gałąź { $branch }
...
command-partial-index = Zindeksowano tylko część tego projektu
command-more-matches = Wyświetlono {$shown} z {$total} dopasowań
...
ex-quit = Zamknij bez zapisywania
ex-quit-force = Zamknij i odrzuć zmiany
ex-nohighlight = Wyczyść podświetlenie wyszukiwania
ex-delete = Usuń wiersz
ex-yank = Skopiuj wiersz
ex-substitute = Zamień w wierszu
...
common-collapse-all = Zwiń wszystko
...
command-browser-open-goto-line = Przeglądarka > Pasek > Przejdź do wiersza
command-browser-open-indentation = Przeglądarka > Pasek > Wybierz wcięcie
command-browser-open-line-ending = Przeglądarka > Pasek > Wybierz znak końca wiersza
command-browser-open-encoding = Przeglądarka > Pasek > Wybierz kodowanie
command-browser-open-reopen-with-encoding = Przeglądarka > Pasek > Otwórz ponownie z kodowaniem
command-browser-open-save-with-encoding = Przeglądarka > Pasek > Zapisz z kodowaniem
...
agent-change-model = Zmień model
...
agent-tool-calls = { $count ->
    [one] { $count } wywołanie narzędzia
   *[other] { $count } wywołań narzędzi
}
...
agent-option-default = { $value } (domyślnie)
agent-tool-calling = Wywoływanie narzędzia
...
editor-status-goto-title = Przejdź do wiersza
editor-status-goto-placeholder = Numer wiersza
editor-status-pick-placeholder = Wpisz, aby filtrować
editor-status-indent-title = Wybierz wcięcie
editor-status-eol-title = Wybierz sekwencję końca wiersza
editor-status-encoding-title = Wybierz kodowanie
editor-status-encoding-reopen = Otwórz ponownie z kodowaniem
editor-status-encoding-save = Zapisz z kodowaniem
...
editor-go-to-type-definition = Przejdź do definicji typu
editor-go-to-implementation = Przejdź do implementacji
editor-rename-symbol = Zmień nazwę symbolu
editor-change-all-occurrences = Zmień wszystkie wystąpienia
editor-format-document = Formatuj dokument
editor-format-selection = Formatuj zaznaczenie
editor-code-action = Refaktoryzuj…
editor-cut = Wytnij
editor-copy = Kopiuj
editor-paste = Wklej
editor-command-palette = Paleta poleceń…
editor-unsaved = niezapisane
editor-find-placeholder = Szukaj
editor-find-no-results = Brak wyników
editor-find-previous = Poprzednie dopasowanie
editor-find-next = Następne dopasowanie
editor-find-close = Zamknij wyszukiwanie
editor-find-regex = Użyj wyrażenia regularnego
editor-find-case = Uwzględnij wielkość liter
editor-find-in-files-placeholder = Szukaj w plikach
editor-find-whole-word = Całe słowo
editor-search = Szukaj
editor-search-clear = Wyczyść wyniki wyszukiwania
editor-search-idle = Naciśnij Enter, aby wyszukać
editor-search-summary = Wyniki: { $results } w { $files } plikach
editor-search-summary-capped = Pierwsze { $results } wyników w { $files } plikach
editor-show-explorer = Pokaż eksplorator
editor-show-search = Pokaż wyszukiwanie
...
editor-rename-failed = Zmiana nazwy nie powiodła się
...
git-stage-file = Dodaj plik do przechowalni
git-unstage = Usuń z poczekalni
git-confirm-discard = Potwierdź odrzucenie
git-discard-file = Odrzuć zmiany
git-commit-message = komunikat commita
git-commit = Commit ({ $count })
git-push = ↑ Wypchnij
git-loading-diff = Wczytywanie diffu…
git-no-changes = Brak zmian do pokazania
git-stage-hunk = Dodaj fragment
git-revert-hunk = Przywróć fragment
...
knowledge-search = Szukaj wiedzy
knowledge-no-match = Brak pasujących notatek
knowledge-open-linked-note = Otwórz połączoną notatkę
knowledge-create-linked-note = Utwórz połączoną notatkę
editor-properties = Właściwości
editor-no-properties = Brak właściwości
editor-add-tags = Dodaj tagi
editor-add-property = Dodaj nieruchomość
editor-change-property-type = Zmień typ właściwości
editor-add-tag = Dodaj tag
editor-add-item = Dodaj element
editor-linked-note = Połączona notatka
editor-property-value = Wartość
editor-delete-property = Usuń właściwość
editor-property-kind-text = Tekst
editor-property-kind-number = Numer
editor-property-kind-checkbox = Pole wyboru
...
mobile-pair-title = Połącz się z komputerem Mac
mobile-pair-subtitle = W Vmux otwórz Remote i wybierz Connect device.
mobile-pair-scan = Zeskanuj kod QR
mobile-pair-show-link = Wpisz link parowania ręcznie
mobile-pair-hide-link = Ukryj link parowania
mobile-pair-link-placeholder = Wklej link parowania
mobile-pair-connect = Połącz
mobile-pair-connecting = Łączenie…
mobile-pair-disconnect = Rozłącz
mobile-status-connected = Połączono
mobile-status-reaching = Szukanie komputera Mac…
mobile-status-disconnected = Nie połączono

mobile-url-invalid = Adres URL parowania jest nieprawidłowy.
mobile-url-no-address = Adres URL parowania nie zawiera adresu serwera.
mobile-url-bad-address = Adres URL parowania zawiera nieprawidłowy adres serwera.
mobile-url-no-token = Adres URL parowania nie zawiera tokenu.
mobile-url-scheme = Adres URL parowania musi używać HTTPS lub HTTP.
mobile-url-paste-full = Wklej pełny adres URL parowania wyświetlony przez Vmux na komputerze Mac.

mobile-error-pairing-outdated = To parowanie jest nieaktualne. Zeskanuj ponownie kod QR na komputerze Mac.
mobile-error-pairing-expired = Parowanie wygasło. Zeskanuj ponownie kod QR na komputerze Mac.
mobile-error-pairing-lost = Parowanie wygasło. Sparuj ponownie z komputerem Mac.
mobile-error-token-rejected = Token parowania został odrzucony.
mobile-error-stack-missing = Aplikacja na komputerze otworzyła czat, ale jego stos się nie pojawił.
mobile-error-not-offered = Twój Mac jeszcze tego nie udostępnia.
mobile-error-remote-disabled = Remote jest wyłączony na Twoim komputerze Mac.
mobile-error-session-gone = Ta sesja już nie istnieje.
mobile-error-no-desktop = Otwórz okno Vmux na komputerze Mac.
mobile-error-refused = Twój Mac nie mógł tego zrobić.
mobile-error-no-answer = Brak odpowiedzi z { $address } po { $seconds } s.
...
mobile-chat-back = Wstecz
mobile-chat-back-to-stacks = Powrót do stosów
mobile-chat-no-messages = Brak wiadomości.
mobile-chat-no-session = Brak aktywnej sesji
mobile-chat-placeholder = Napisz do agenta…
mobile-chat-send = Wyślij
mobile-chat-stop = Zatrzymaj

mobile-qr-title = Zeskanuj kod QR wyświetlony przez Vmux
mobile-qr-cancel = Anuluj
mobile-qr-unavailable = Skaner QR jest niedostępny.
mobile-qr-camera-unavailable = Aparat niedostępny. Wpisz zamiast tego link parowania.
mobile-qr-camera-failed = Nie udało się uruchomić aparatu: { $error }
mobile-qr-camera-input-unavailable = Wejście aparatu jest niedostępne.
mobile-qr-unsupported-device = Skanowanie kodów QR jest niedostępne na tym urządzeniu.
mobile-qr-unsupported-platform = Skanowanie kodów QR nie jest jeszcze dostępne na tej platformie. Wpisz zamiast tego link parowania.
mobile-error-unexpected-answer = Twój Mac odpowiedział nieoczekiwanie.
mobile-error-connection-dropped = Połączenie z komputerem Mac zostało zerwane.
...
agent-project-select = Wybierz projekt
agent-project-choose = Wybór projektu
agent-project-choose-another = Wybierz inny folder…
agent-project-none = Brak projektów
...
resume-column-project = Projekt
resume-column-branch = Gałąź
resume-column-updated = Zaktualizowano
resume-age-now = teraz
resume-age-minutes = { $count } min
resume-age-hours = { $count } godz.
resume-age-days = { $count } dn.
resume-age-weeks = { $count } tyg.
resume-age-months = { $count } mies.
resume-age-years = { $count } l.
