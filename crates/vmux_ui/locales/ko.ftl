...
mcp-command-description = MCP 서버 관리
mcp-linear-description = 이슈, 프로젝트 및 팀
...
composer-status-running = 실행 중
composer-status-approval = 승인 대기
composer-status-starting = 시작 중
composer-status-error = 오류
composer-status-subagents = 활성 서브에이전트
composer-status-tasks-title = 열린 작업
composer-status-tasks = 작업 { $count }개
composer-status-queued-title = 대기 중인 프롬프트
composer-status-queued = 대기 { $count }개
composer-uncommitted-changes = 커밋되지 않은 변경 사항
composer-commits-ahead = 업스트림보다 앞선 커밋
composer-no-git = Git 없음
composer-git = Git
composer-git-repository = Git 저장소
composer-branch-name = 브랜치 { $branch }
...
command-partial-index = 이 프로젝트의 일부만 색인되었습니다
command-more-matches = {$total}개 중 {$shown}개를 표시하고 있습니다
...
ex-quit = 저장하지 않고 닫기
ex-quit-force = 변경 사항을 버리고 닫기
ex-nohighlight = 검색 강조 해제
ex-delete = 줄 삭제
ex-yank = 줄 복사
ex-substitute = 줄에서 치환
...
command-browser-open-goto-line = Browser > 막대 > 줄로 이동
command-browser-open-indentation = Browser > 막대 > 들여쓰기 선택
command-browser-open-line-ending = Browser > 막대 > 줄 바꿈 문자 선택
command-browser-open-encoding = Browser > 막대 > 인코딩 선택
command-browser-open-reopen-with-encoding = Browser > 막대 > 인코딩을 지정하여 다시 열기
command-browser-open-save-with-encoding = Browser > 막대 > 인코딩을 지정하여 저장
...
agent-option-default = { $value } (기본값)
agent-tool-calling = 도구 호출 중
...
editor-status-goto-title = 줄로 이동
editor-status-goto-placeholder = 줄 번호
editor-status-pick-placeholder = 입력하여 필터링
editor-status-indent-title = 들여쓰기 선택
editor-status-eol-title = 줄 끝 시퀀스 선택
editor-status-encoding-title = 인코딩 선택
editor-status-encoding-reopen = 인코딩하여 다시 열기
editor-status-encoding-save = 인코딩하여 저장
editor-cannot-open = 이 파일을 열 수 없습니다
editor-edit-failed = 이름 바꾸기에 실패했습니다
editor-go-to-declaration = 선언으로 이동
editor-go-to-type-definition = 형식 정의로 이동
editor-go-to-implementation = 구현으로 이동
editor-rename-symbol = 기호 이름 바꾸기
...
editor-find-placeholder = 찾기
editor-find-no-results = 결과 없음
editor-find-previous = 이전 일치
editor-find-next = 다음 일치
editor-find-close = 찾기 닫기
editor-find-regex = 정규식 사용
editor-find-case = 대/소문자 구분
editor-find-in-files-placeholder = 파일에서 검색
editor-find-whole-word = 단어 단위로 일치
editor-search = 검색
editor-search-clear = 검색 결과 지우기
editor-search-idle = Enter 키를 눌러 검색
editor-search-summary = 파일 { $files }개에서 결과 { $results }개
editor-search-summary-capped = 파일 { $files }개에서 처음 { $results }개 결과
editor-show-explorer = 탐색기 표시
editor-show-search = 검색 표시
...
editor-outline = 개요
editor-close-directory = 디렉터리 닫기
editor-breadcrumb = 탐색 경로
editor-breadcrumb-hidden = 숨겨진 폴더 표시
editor-breadcrumb-empty = 항목이 없습니다
...
editor-rename-failed = 이름 바꾸기 실패
...
git-stage-file = 파일 스테이징
git-unstage = 스테이징 해제
git-confirm-discard = 폐기 확인
git-discard-file = 변경 사항 폐기
git-commit-message = 커밋 메시지
git-commit = 커밋({ $count })
git-push = ↑ 푸시
git-loading-diff = diff 로드 중…
git-no-changes = 표시할 변경 사항 없음
git-stage-hunk = 헝크 스테이징
git-revert-hunk = 헝크 되돌리기
...
knowledge-no-match = 일치하는 메모 없음
knowledge-open-linked-note = 연결된 노트 열기
knowledge-create-linked-note = 연결된 노트 만들기
editor-properties = 속성
editor-no-properties = 속성 없음
editor-add-tags = 태그 추가
editor-add-property = 속성 추가
editor-change-property-type = 속성 유형 변경
editor-add-tag = 태그 추가
editor-add-item = 항목 추가
editor-linked-note = 연결된 노트
editor-property-value = 값
editor-delete-property = 속성 삭제
editor-property-kind-text = 텍스트
editor-property-kind-number = 숫자
editor-property-kind-checkbox = 체크박스
editor-property-kind-date = 날짜
...
mobile-pair-title = Mac에 연결
mobile-pair-subtitle = Vmux에서 Remote를 열고 기기 연결을 선택하세요.
mobile-pair-scan = QR 코드 스캔
mobile-pair-show-link = 페어링 링크 직접 입력
mobile-pair-hide-link = 페어링 링크 숨기기
mobile-pair-link-placeholder = 페어링 링크 붙여넣기
mobile-pair-connect = 연결
mobile-pair-connecting = 연결 중…
mobile-pair-disconnect = 연결 해제
mobile-status-connected = 연결됨
mobile-status-reaching = Mac에 연결하는 중…
mobile-status-disconnected = 연결되지 않음

mobile-url-invalid = 페어링 URL이 올바르지 않습니다.
mobile-url-no-address = 페어링 URL에 서버 주소가 없습니다.
mobile-url-bad-address = 페어링 URL의 서버 주소가 올바르지 않습니다.
mobile-url-no-token = 페어링 URL에 토큰이 없습니다.
mobile-url-scheme = 페어링 URL은 HTTPS 또는 HTTP를 사용해야 합니다.
mobile-url-paste-full = Mac의 Vmux에 표시된 페어링 URL 전체를 붙여넣으세요.

mobile-error-pairing-outdated = 이 페어링은 오래되었습니다. Mac의 QR을 다시 스캔하세요.
mobile-error-pairing-expired = 페어링이 만료되었습니다. Mac의 QR을 다시 스캔하세요.
mobile-error-pairing-lost = 페어링이 만료되었습니다. Mac과 다시 페어링하세요.
mobile-error-token-rejected = 페어링 토큰이 거부되었습니다.
mobile-error-stack-missing = 데스크톱에서 채팅은 열렸지만 스택이 나타나지 않았습니다.
mobile-error-not-offered = Mac에서 아직 지원하지 않는 기능입니다.
mobile-error-remote-disabled = Mac에서 Remote가 꺼져 있습니다.
mobile-error-session-gone = 해당 세션이 사라졌습니다.
mobile-error-no-desktop = Mac에서 Vmux 창을 여세요.
mobile-error-refused = Mac이 요청을 처리하지 못했습니다.
mobile-error-no-answer = { $address }에서 { $seconds }초 동안 응답이 없습니다.
...
mobile-chat-no-session = 활성 세션 없음
...
mobile-qr-cancel = 취소
mobile-qr-unavailable = QR 스캐너를 사용할 수 없습니다.
mobile-qr-camera-unavailable = 카메라를 사용할 수 없습니다. 대신 페어링 링크를 입력하세요.
mobile-qr-camera-failed = 카메라를 열 수 없습니다: { $error }
mobile-qr-camera-input-unavailable = 카메라 입력을 사용할 수 없습니다.
mobile-qr-unsupported-device = 이 기기에서는 QR 스캔을 사용할 수 없습니다.
mobile-qr-unsupported-platform = 이 플랫폼에서는 아직 QR 스캔을 사용할 수 없습니다. 대신 페어링 링크를 입력하세요.
mobile-error-unexpected-answer = Mac이 예기치 않은 응답을 보냈습니다.
mobile-error-connection-dropped = Mac과의 연결이 끊어졌습니다.
...
agent-project-select = 프로젝트 선택
agent-project-choose = 프로젝트 고르기
agent-project-choose-another = 다른 폴더 선택…
agent-project-none = 프로젝트가 없습니다
...
resume-column-agent = 에이전트
resume-column-project = 프로젝트
resume-column-branch = 브랜치
resume-column-updated = 업데이트
resume-age-now = 방금
resume-age-minutes = { $count }분 전
resume-age-hours = { $count }시간 전
resume-age-days = { $count }일 전
resume-age-weeks = { $count }주 전
resume-age-months = { $count }개월 전
resume-age-years = { $count }년 전
