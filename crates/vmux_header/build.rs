...
fn main() {
    ...
    WebviewAppBuilder::new(manifest_dir, "vmux_header", "vmux_header_app")
        .track_manifest_rel_paths(&["tailwind.config.js", "assets/theme.css"])
        .dx_extra_args(&["--bin", "vmux_header_app", "--features", "web"])
        .cef_finalize(CefEmbeddedWebviewFinalize {
            strip_uncompiled_tailwind_css: true,
        })
        .tailwind_postprocess_after_dx(&["index-dxh", "header-dxh"])
        .run("vmux_header");
}
