...
mcp-command-description = 管理 MCP 伺服器
mcp-linear-description = 議題、專案和團隊
...
vault-recovery-key-create-failed = 無法建立恢復密鑰。
vault-recovery-key-save = 立即儲存此密鑰。 Vmux 無法再次顯示它。
vault-recovery-key-saved = 我保存了
vault-recovery-key-placeholder = 貼上恢復金鑰
vault-recovery-key-unlock = 開鎖
vault-recovery-key-invalid = 此恢復密鑰無法解鎖保管庫。
vault-recovery-key-ready = 已配置恢復密鑰
vault-recovery-key-upload-pending = 密鑰保存在本地，但上傳失敗。確保安全並按“同步”重試。
vault-auto-sync = 自動備份
vault-backup-failed = 自動備份失敗。按“同步”重試。
vault-backup-failed-short = 備份失敗
vault-recovery-key-copy-hint = 點選複製
vault-recovery-key-copied = 已複製
vault-recovery-key-verify = 將其保存在密碼管理器中，然後將其貼到下面進行驗證。
vault-recovery-key-verify-placeholder = 貼上保存的復原金鑰
vault-recovery-key-mismatch = 恢復密鑰不匹配。
...
vault-connect = 連接
vault-github = GitHub
vault-github-description = 建立私有儲存庫或選擇現有的保管庫。
vault-connect-github = 連接 GitHub
vault-connected-as = 連接為 { $name }
vault-use-repository = 使用
vault-repository-name = 儲存庫名稱
vault-cloud-folder = 雲端資料夾
vault-cloud-folder-description = 使用 Google Drive、iCloud Drive、Dropbox 或 OneDrive 中的資料夾。
vault-choose-folder = 選擇資料夾
vault-result-github-connected = GitHub · { common-done }
vault-result-folder-connected = 資料夾 · { common-done }
vault-private = 私有倉庫
vault-public-warning = 儲存庫歷史記錄和加密資料將公開。
...
start-title = 開始
start-tagline = 一句提示，什麼都能完成。

agents-title = Agent
agents-search = 搜尋 ACP 與 CLI Agent…
agents-empty = 找不到符合的 Agent
agents-empty-detail = 試試名稱、執行環境或 ACP/CLI。
...
lsp-title = 語言伺服器
lsp-search = 搜尋語言伺服器、linter、formatter…
lsp-loading = 正在載入目錄…
lsp-empty = 找不到符合的語言伺服器
lsp-empty-detail = 試試其他語言、linter 或 formatter。
lsp-needs = 需要 { $tool }
...
services-empty = 沒有執行中的程序
services-filter = 篩選程序…
services-no-match = 找不到符合的程序
...
history-clear-confirm = 要清除所有記錄嗎？
history-clear-warning = 此動作無法復原。
history-cancel = 取消
history-today = 今天
...
settings-update-unavailable-hint = 此建置未包含更新程式。
...
settings-update-ready-hint = Vmux { $version } 已準備就緒。重新啟動即可套用。
...
settings-saved = 已儲存
settings-record-key = 點按以錄製新的按鍵組合
...
composer-choose-agent = 選擇代理
composer-create-worktree = + 工作樹
composer-create-worktree-title = 為此專案建立或選擇工作樹
composer-status-running = 執行中
composer-status-approval = 待核准
composer-status-starting = 啟動中
composer-status-error = 錯誤
composer-status-subagents = 作用中的子代理
composer-status-tasks-title = 未完成工作
composer-status-tasks = { $count } 項工作
composer-status-queued-title = 排隊的提示
composer-status-queued = 排隊 { $count } 項
composer-uncommitted-changes = 未提交的變更
composer-commits-ahead = 領先上游的提交
composer-no-git = 無 Git
composer-git = Git
composer-git-repository = Git 儲存庫
composer-branch-name = 分支 { $branch }
...
command-partial-index = 僅索引了此專案的一部分
command-more-matches = 顯示 {$total} 個相符項目中的 {$shown} 個
...
ex-write-quit = 儲存並關閉
ex-quit = 不儲存並關閉
ex-quit-force = 捨棄變更並關閉
ex-nohighlight = 清除搜尋醒目提示
ex-delete = 刪除該行
ex-yank = 複製該行
ex-substitute = 在該行內取代
...
schema-shortcuts-detail = 唯讀檢視。若要變更綁定，請直接編輯 settings.ron。
schema-terminal = 終端機
schema-browser = 瀏覽器
schema-mode = 模式
schema-mode-detail = 網頁的配色。裝置模式會跟隨系統。
schema-device = 裝置
schema-light = 淺色
schema-dark = 深色
schema-language = 語言
schema-language-detail = 使用系統、en-US、ja，或任何有對應 ~/.vmux/locales/<tag>.ftl 目錄的 BCP 47 標籤。
schema-auto-update = 自動更新
schema-auto-update-detail = 啟動時及每小時檢查並安裝更新。
schema-startup-url = 啟動 URL
schema-startup-url-detail = 留空則開啟指令列提示。
schema-search-engine = 搜尋引擎
schema-search-engine-detail = 用於從「開始」和指令列進行網頁搜尋。
schema-window = 視窗
schema-pane = 窗格
...
schema-chord-timeout-detail = 和弦前置鍵到期前的毫秒數。
schema-bindings = 綁定
schema-confirm-close = 關閉前確認
schema-confirm-close-detail = 關閉仍有程序執行的終端機前先提示。
schema-default-theme = 預設主題
schema-default-theme-detail = 主題清單中作用中主題的名稱。
...
schema-tidy-files-max = 檔案整理臨界值
schema-tidy-files-auto = 自動整理檔案
schema-app-providers = App 供應商
schema-provider = 供應商
schema-kind = 類型
schema-models = 模型
...
layout-bookmark-in = 加入 { $folder } 的書籤
...
services-uptime-hours = { $hours } 小時 { $minutes } 分
services-uptime-days = { $days } 天 { $hours } 小時
...
command-close-tab = 佈局 > 標籤頁 > 關閉標籤頁
command-new-task = 佈局 > 標籤頁 > 新增任務…
command-next-tab = 佈局 > 標籤頁 > 下一個標籤頁
command-prev-tab = 佈局 > 標籤頁 > 上一個標籤頁
command-rename-tab = 佈局 > 標籤頁 > 重新命名標籤頁
command-tab-select-1 = 佈局 > 標籤頁 > 選取標籤頁 1
command-tab-select-2 = 佈局 > 標籤頁 > 選取標籤頁 2
command-tab-select-3 = 佈局 > 標籤頁 > 選取標籤頁 3
command-tab-select-4 = 佈局 > 標籤頁 > 選取標籤頁 4
command-tab-select-5 = 佈局 > 標籤頁 > 選取標籤頁 5
command-tab-select-6 = 佈局 > 標籤頁 > 選取標籤頁 6
command-tab-select-7 = 佈局 > 標籤頁 > 選取標籤頁 7
command-tab-select-8 = 佈局 > 標籤頁 > 選取標籤頁 8
command-tab-select-last = 佈局 > 標籤頁 > 選取最後一個標籤頁
...
command-resize-pane-left = 佈局 > 窗格 > 向左調整窗格大小
command-resize-pane-right = 佈局 > 窗格 > 向右調整窗格大小
command-resize-pane-up = 佈局 > 窗格 > 向上調整窗格大小
command-resize-pane-down = 佈局 > 窗格 > 向下調整窗格大小
...
command-space-open = 佈局 > Space > Spaces
...
command-open-in-pane-top = 瀏覽器 > 開啟 > 在上方窗格中開啟
command-open-in-pane-right = 瀏覽器 > 開啟 > 在右側窗格中開啟
command-open-in-pane-bottom = 瀏覽器 > 開啟 > 在下方窗格中開啟
command-open-in-pane-left = 瀏覽器 > 開啟 > 在左側窗格中開啟
command-open-in-new-tab = 瀏覽器 > 開啟 > 在新標籤頁中開啟
command-open-in-new-space = 瀏覽器 > 開啟 > 在新 Space 中開啟
...
command-browser-open-command-bar = 瀏覽器 > 列 > 命令列
command-browser-open-page-in-command-bar = 瀏覽器 > 列 > 編輯頁面
command-browser-open-path-bar = 瀏覽器 > 列 > 路徑導覽器
command-browser-open-commands = 瀏覽器 > 列 > 命令
command-browser-open-history = 瀏覽器 > 列 > 瀏覽記錄
command-browser-open-goto-line = 瀏覽器 > 列 > 前往行
command-browser-open-indentation = 瀏覽器 > 列 > 選擇縮排
command-browser-open-line-ending = 瀏覽器 > 列 > 選擇行尾序列
command-browser-open-encoding = 瀏覽器 > 列 > 選擇編碼
command-browser-open-reopen-with-encoding = 瀏覽器 > 列 > 以編碼重新開啟
command-browser-open-save-with-encoding = 瀏覽器 > 列 > 以編碼儲存
...
agent-working-conjuring = 構思中
agent-working-cooking = 處理中
agent-working-brewing = 醞釀中
agent-working-musing = 思索中
agent-working-ruminating = 反覆思考中
agent-working-scheming = 規劃中
agent-working-synthesizing = 彙整中
agent-working-tinkering = 調整中
agent-working-churning = 推進中
...
editor-status-goto-title = 移至行
editor-status-goto-placeholder = 行號
editor-status-pick-placeholder = 輸入以篩選
editor-status-indent-title = 選取縮排
editor-status-eol-title = 選取行尾字元
editor-status-encoding-title = 選取編碼
editor-status-encoding-reopen = 以編碼重新開啟
editor-status-encoding-save = 以編碼儲存
editor-cannot-open = 無法開啟此檔案
editor-edit-failed = 重新命名失敗
editor-go-to-declaration = 移至宣告
editor-go-to-type-definition = 移至型別定義
editor-go-to-implementation = 移至實作
editor-rename-symbol = 重新命名符號
editor-change-all-occurrences = 變更所有相符項
editor-format-document = 格式化文件
editor-format-selection = 格式化選取範圍
editor-code-action = 重構…
editor-cut = 剪下
editor-copy = 複製
editor-paste = 貼上
editor-command-palette = 命令選擇區…
editor-unsaved = 未儲存
editor-find-placeholder = 尋找
editor-find-no-results = 無結果
editor-find-previous = 上一個相符項
editor-find-next = 下一個相符項
editor-find-close = 關閉尋找
editor-find-regex = 使用正規表示式
editor-find-case = 區分大小寫
editor-find-in-files-placeholder = 在檔案中搜尋
editor-find-whole-word = 全字比對
editor-search = 搜尋
editor-search-clear = 清除搜尋結果
editor-search-idle = 按 Enter 搜尋
editor-search-summary = 在 { $files } 個檔案中找到 { $results } 個結果
editor-search-summary-capped = 在 { $files } 個檔案中顯示前 { $results } 個結果
editor-show-explorer = 顯示檔案總管
editor-show-search = 顯示搜尋
...
editor-open-externally = 以外部程式開啟
editor-changed-line = 已變更行
editor-go-to-definition = 前往定義
editor-find-references = 尋找參考
...
editor-failed-decode-image = 無法解碼圖片
editor-preview-large-image = 圖片（太大，無法預覽）
editor-preview-binary = 二進位檔
...
git-stage-file = 暫存檔案
git-unstage = 取消暫存
git-confirm-discard = 確認捨棄
git-discard-file = 捨棄變更
git-commit-message = 提交訊息
git-commit = 提交（{ $count }）
git-push = ↑ 推送
git-loading-diff = 正在載入差異…
git-no-changes = 沒有可顯示的變更
git-stage-hunk = 暫存程式碼區塊
git-revert-hunk = 還原程式碼區塊
...
setup-homebrew-required = 需要 Homebrew 才能安裝 { $command }，但尚未設定。Vmux 會先安裝 Homebrew，再安裝 { $name }。
setup-terminal-instructions = 在終端機中，按 Return 開始，接著依提示輸入你的 Mac 密碼。
setup-command-missing = Vmux 開啟此頁面，是因為尚未安裝本機 { $command } 命令。執行下方命令即可取得。
setup-install-failed = 安裝未完成。請查看終端機中的詳細資訊，然後重試。
...
command-group-space = 空間
command-group-navigation = 導覽
command-group-open = 開啟
command-group-view = 檢視
command-group-bar = 工具列

menu-close-vmux = 關閉 Vmux

agents-terminal-coding-agent = 終端機型程式碼代理
knowledge-search = 搜尋知識
knowledge-no-match = 沒有相符的註釋
knowledge-open-linked-note = 打開連結的筆記
knowledge-create-linked-note = 建立連結筆記
editor-properties = 特性
editor-no-properties = 無屬性
editor-add-tags = 添加標籤
editor-add-property = 新增屬性
editor-change-property-type = 更改屬性類型
editor-add-tag = 添加標籤
editor-add-item = 新增項目
editor-linked-note = 連結筆記
editor-property-value = 價值
editor-delete-property = 刪除屬性
editor-property-kind-text = 文字
editor-property-kind-number = 數位
editor-property-kind-checkbox = 複選框
editor-property-kind-date = 日期
...
mobile-pair-title = 連接到你的 Mac
mobile-pair-subtitle = 在 Vmux 中開啟 Remote，然後選擇 Connect device。
mobile-pair-scan = 掃描 QR 碼
mobile-pair-show-link = 手動輸入配對連結
mobile-pair-hide-link = 隱藏配對連結
...
mobile-status-reaching = 正在聯繫你的 Mac…
mobile-status-disconnected = 未連線

mobile-url-invalid = 配對 URL 無效。
mobile-url-no-address = 配對 URL 沒有伺服器位址。
mobile-url-bad-address = 配對 URL 的伺服器位址無效。
mobile-url-no-token = 配對 URL 沒有權杖。
mobile-url-scheme = 配對 URL 必須使用 HTTPS 或 HTTP。
mobile-url-paste-full = 請貼上 Vmux 在你的 Mac 上顯示的完整配對 URL。

mobile-error-pairing-outdated = 這個配對已過時。請重新掃描 Mac 上的 QR 碼。
mobile-error-pairing-expired = 配對已過期。請重新掃描 Mac 上的 QR 碼。
mobile-error-pairing-lost = 配對已過期。請重新與 Mac 配對。
mobile-error-token-rejected = 配對權杖遭拒。
mobile-error-stack-missing = 桌面版已開啟聊天，但它的堆疊沒有出現。
mobile-error-not-offered = 你的 Mac 尚未提供這項功能。
mobile-error-remote-disabled = 你的 Mac 上 Remote 已關閉。
mobile-error-session-gone = 該工作階段已不存在。
mobile-error-no-desktop = 請在 Mac 上開啟 Vmux 視窗。
mobile-error-refused = 你的 Mac 無法執行這項操作。
mobile-error-no-answer = { $seconds } 秒後仍未收到 { $address } 的回應。

mobile-start-stacks = 堆疊
mobile-start-no-stacks = 沒有開啟的堆疊
...
mobile-chat-no-session = 沒有進行中的工作階段
...
mobile-qr-camera-unavailable = 相機無法使用。請改為輸入配對連結。
mobile-qr-camera-failed = 無法開啟相機：{ $error }
mobile-qr-camera-input-unavailable = 相機輸入無法使用。
mobile-qr-unsupported-device = 這部裝置不支援 QR 掃描。
mobile-qr-unsupported-platform = 這個平台尚不支援 QR 掃描。請改為輸入配對連結。
mobile-error-unexpected-answer = 你的 Mac 回應異常。
mobile-error-connection-dropped = 與你的 Mac 的連線已中斷。
...
agent-project-select = 選擇專案
agent-project-choose = 選擇專案
agent-project-choose-another = 選擇其他資料夾…
agent-project-none = 尚無專案
agent-project-loading-branches = 正在載入分支…
agent-project-no-branches = 沒有分支
agent-project-open-worktree = 開啟此工作樹
agent-project-branch-change = 相對於基礎分支的變更
agent-project-create-worktree = 在此分支建立工作樹
resume-column-agent = 智慧代理
resume-column-project = 專案
resume-column-branch = 分支
resume-column-updated = 更新於
resume-age-now = 剛剛
resume-age-minutes = { $count } 分鐘前
resume-age-hours = { $count } 小時前
resume-age-days = { $count } 天前
resume-age-weeks = { $count } 週前
resume-age-months = { $count } 個月前
resume-age-years = { $count } 年前
