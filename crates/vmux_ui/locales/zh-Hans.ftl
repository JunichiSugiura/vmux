...
mcp-command-description = 管理 MCP 服务器
mcp-linear-description = 议题、项目和团队
...
composer-status-running = 运行中
composer-status-approval = 待批准
composer-status-starting = 启动中
composer-status-error = 错误
composer-status-subagents = 活动子代理
composer-status-tasks-title = 未完成任务
composer-status-tasks = { $count } 项任务
composer-status-queued-title = 排队的提示
composer-status-queued = 排队 { $count } 项
composer-uncommitted-changes = 未提交的更改
composer-commits-ahead = 领先上游的提交
composer-no-git = 无 Git
composer-git = Git
composer-git-repository = Git 仓库
...
command-partial-index = 仅索引了该项目的一部分
command-more-matches = 显示 {$total} 个匹配项中的 {$shown} 个
command-placeholder = 输入 URL、搜索标签页，或输入 > 打开命令…
command-ex-placeholder = 输入 Ex 命令…
palette-mode-ex = Ex
palette-mode-command = 命令
palette-mode-slash = 命令
palette-mode-path = 文件
ex-write = 保存文件
ex-write-quit = 保存并关闭
ex-quit = 不保存并关闭
ex-quit-force = 放弃更改并关闭
ex-nohighlight = 清除搜索高亮
ex-delete = 删除该行
ex-yank = 复制该行
ex-substitute = 在该行内替换
...
layout-project-forget = 忘记此项目
layout-project-activate = 在此打开新标签页
...
command-browser-open-goto-line = 浏览器 > 栏 > 转到行
command-browser-open-indentation = 浏览器 > 栏 > 选择缩进
command-browser-open-line-ending = 浏览器 > 栏 > 选择行尾序列
command-browser-open-encoding = 浏览器 > 栏 > 选择编码
command-browser-open-reopen-with-encoding = 浏览器 > 栏 > 以编码重新打开
command-browser-open-save-with-encoding = 浏览器 > 栏 > 以编码保存
...
agent-option-default = { $value }（默认）
agent-tool-calling = 正在调用工具
...
editor-status-goto-title = 转到行
editor-status-goto-placeholder = 行号
editor-status-pick-placeholder = 键入以筛选
editor-status-indent-title = 选择缩进
editor-status-eol-title = 选择行尾序列
editor-status-encoding-title = 选择编码
editor-status-encoding-reopen = 通过编码重新打开
editor-status-encoding-save = 通过编码保存
...
editor-find-no-results = 无结果
editor-find-previous = 上一个匹配项
editor-find-next = 下一个匹配项
editor-find-close = 关闭查找
editor-find-regex = 使用正则表达式
editor-find-case = 区分大小写
editor-find-in-files-placeholder = 在文件中搜索
editor-find-whole-word = 全字匹配
editor-search = 搜索
editor-search-clear = 清除搜索结果
editor-search-idle = 按 Enter 搜索
editor-search-summary = 在 { $files } 个文件中找到 { $results } 个结果
editor-search-summary-capped = 在 { $files } 个文件中显示前 { $results } 个结果
editor-show-explorer = 显示资源管理器
editor-show-search = 显示搜索
...
editor-explorer-empty = 无文件
editor-open-editors = 打开的编辑器
editor-close-editor = 关闭编辑器
editor-outline = 大纲
editor-close-directory = 关闭目录
editor-breadcrumb = 导航路径
editor-breadcrumb-hidden = 显示隐藏的文件夹
editor-breadcrumb-empty = 没有内容
...
git-stage-file = 暂存文件
git-unstage = 取消暂存
git-confirm-discard = 确认放弃
git-discard-file = 放弃更改
...
knowledge-no-match = 没有匹配的注释
knowledge-open-linked-note = 打开链接的笔记
knowledge-create-linked-note = 创建链接笔记
editor-properties = 特性
editor-no-properties = 无属性
editor-add-tags = 添加标签
editor-add-property = 添加属性
editor-change-property-type = 更改属性类型
editor-add-tag = 添加标签
editor-add-item = 添加项目
editor-linked-note = 链接笔记
editor-property-value = 价值
editor-delete-property = 删除属性
editor-property-kind-text = 文本
editor-property-kind-number = 数字
editor-property-kind-checkbox = 复选框
...
mobile-pair-title = 连接到你的 Mac
mobile-pair-subtitle = 在 Vmux 中打开 Remote，然后选择 Connect device。
mobile-pair-scan = 扫描 QR 码
mobile-pair-show-link = 手动输入配对链接
mobile-pair-hide-link = 隐藏配对链接
mobile-pair-link-placeholder = 粘贴配对链接
mobile-pair-connect = 连接
mobile-pair-connecting = 正在连接…
mobile-pair-disconnect = 断开连接
mobile-status-connected = 已连接
mobile-status-reaching = 正在连接你的 Mac…
mobile-status-disconnected = 未连接

mobile-url-invalid = 配对 URL 无效。
mobile-url-no-address = 配对 URL 中没有服务器地址。
mobile-url-bad-address = 配对 URL 中的服务器地址无效。
mobile-url-no-token = 配对 URL 中没有令牌。
mobile-url-scheme = 配对 URL 必须使用 HTTPS 或 HTTP。
mobile-url-paste-full = 粘贴 Mac 上 Vmux 显示的完整配对 URL。

mobile-error-pairing-outdated = 此配对已过时。请重新扫描 Mac 上的 QR 码。
mobile-error-pairing-expired = 配对已过期。请重新扫描 Mac 上的 QR 码。
mobile-error-pairing-lost = 配对已过期。请重新与 Mac 配对。
mobile-error-token-rejected = 配对令牌被拒绝。
mobile-error-stack-missing = 桌面端已打开聊天，但它的堆栈没有出现。
mobile-error-not-offered = 你的 Mac 暂不提供此功能。
mobile-error-remote-disabled = 你的 Mac 上 Remote 已关闭。
mobile-error-session-gone = 该会话已不存在。
mobile-error-no-desktop = 请在 Mac 上打开 Vmux 窗口。
mobile-error-refused = 你的 Mac 无法执行该操作。
mobile-error-no-answer = { $seconds } 秒后仍未收到 { $address } 的响应。
...
mobile-chat-back = 后退
mobile-chat-back-to-stacks = 返回堆栈
mobile-chat-no-messages = 暂无消息。
mobile-chat-no-session = 没有活动会话
...
mobile-qr-cancel = 取消
mobile-qr-unavailable = QR 扫描器不可用。
mobile-qr-camera-unavailable = 摄像头不可用。请改为输入配对链接。
mobile-qr-camera-failed = 无法打开摄像头：{ $error }
mobile-qr-camera-input-unavailable = 摄像头输入不可用。
mobile-qr-unsupported-device = 此设备不支持 QR 扫描。
mobile-qr-unsupported-platform = 此平台尚不支持 QR 扫描。请改为输入配对链接。
mobile-error-unexpected-answer = 你的 Mac 返回了意外的响应。
mobile-error-connection-dropped = 与你的 Mac 的连接已断开。
...
agent-project-select = 选择项目
agent-project-choose = 选择项目
agent-project-choose-another = 选择其他文件夹…
agent-project-none = 暂无项目
...
resume-column-agent = 智能体
resume-column-project = 项目
resume-column-branch = 分支
resume-column-updated = 更新于
resume-age-now = 刚刚
resume-age-minutes = { $count } 分钟前
resume-age-hours = { $count } 小时前
resume-age-days = { $count } 天前
resume-age-weeks = { $count } 周前
resume-age-months = { $count } 个月前
